...
export function AppSidebar() {
  ...
  return <Sidebar className="border-r border-sidebar-border sidebar-glass" collapsible="icon">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative group">
            ...
            <div className="relative w-16 h-16 rounded-xl overflow-hidden bg-gradient-to-br from-sidebar-accent to-sidebar-background flex items-center justify-center p-2 ring-1 ring-white/10 shadow-2xl">
              <img alt="20 Point" className="w-full h-full object-contain drop-shadow-lg" src="/lovable-uploads/a241716c-49eb-4ca9-9a4a-715d42266f19.png" />
            </div>
          </div>
              ...
              return <SidebarMenuItem key={item.title}>
                    <SidebarMenuButton asChild isActive={isActive} tooltip={item.title}>
                      <NavLink to={item.url} end className={`
                          ...
                          ${isActive ? 'bg-gradient-to-r from-primary/20 to-accent/10 text-sidebar-primary shadow-lg shadow-primary/10' : 'hover:bg-sidebar-accent/40 text-sidebar-foreground/70 hover:text-sidebar-foreground'}
                        `} activeClassName="">
                        {isActive && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-gradient-to-b from-primary to-accent rounded-r-full" />}
                        ...
                        {!collapsed && <span className={`font-medium ${isActive ? 'text-sidebar-foreground' : ''}`}>
                            {item.title}
                          </span>}
                      </NavLink>
                    </SidebarMenuButton>
