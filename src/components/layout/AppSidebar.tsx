import { LayoutDashboard, Calendar, LineChart, BookOpen, BarChart3, Upload, TrendingUp } from "lucide-react";
...
export function AppSidebar() {
  ...
  return <Sidebar className="border-r border-sidebar-border bg-sidebar" collapsible="icon">
      ...
      <SidebarFooter className="p-4 border-t border-sidebar-border">
        <NavLink to="/import">
          <Button className={`w-full bg-gradient-to-r from-primary to-accent hover:opacity-90 text-primary-foreground border-none shadow-lg transition-all duration-200 ${collapsed ? 'px-2' : ''}`}>
